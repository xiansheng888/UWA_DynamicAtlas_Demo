...
public class DynamicAtlas
{
    ...
    public void SetTexture(string path, Texture _texture, OnCallBackMetRect callback)
    {
        if (_texture == null || _texture.width > mWidth || _texture.height > mHeight)//不符合规格
        {
            ...
            Debug.LogError("Dynamic SetTexture：texture is null:" + path);
            return;
        }
    ...
    void OnGetImage()
    {
        ...
        ResourcesManager.Instance.LoadResAsync(imageData.path, (string newPath, UnityEngine.Object obj) =>
        {
            Texture2D data = null;
            if (obj != null)
            {
